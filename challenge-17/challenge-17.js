(function(){
	...
	*/
	...
	console.log( text.replace( /Manuel Marques de Sousa/, 'Diego Mandu Nogueira de Freitas' ));
	...
	*/
	...
	var months = [ 'janeiro', 'fevereiro', 'marco', 'abril', 'maio', 'junho', 'julho', 'agosto', 'setembro',
	 'outubro', 'novembro', 'dezembro' ];


	function getMonthNumber( monthName ){
		return monthName.replace( /\w+/, function(total){

			  for(var i = 0; i < months.length; i++ ){
				if(total.toLowerCase() === months[i]){
					if( i < 9 ){
						return '0' + (i + 1)
					}
					return (i + 1);
	...
	*/
	...
	function replaceDate( text ){
		...
		var result = text.replace( regexDate, function( total, day, month, year ){
