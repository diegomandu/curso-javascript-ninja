(function(){
	...
	*/
	...
	function cleanCPF( cpf ){
		var regex = /(\d{3})\D(\d{3})\D(\d+)\D+(\d+)/g;
		return cpf.replace( regex, '$1$2$3$4' );
	}

	

	cpfs = cpfs.map(function( item, index ){
	...
	*/
	...
	cpfs = cpfs.map(function( item, index ){
		...
		return item.replace( regex, '$1.$2.$3-$4'  );
	});
	...
	*/
	...
	console.log('<li></li>", "<li></li>", "<span></span>'.match( /<\w+><\/\w+>/g ));
	...
	*/
	...
	console.log( 'O texto dentro da tag "[NOME DA TAG]" é "[TEXTO]' );

	var rege = /<(\w+)>(\w+[^<]{1,})(<\/\w+>)/g;

	console.log('<h1>Título da página</h1><p>Este é um parágrafo</p><footer>Rodapé</footer>'.replace( 
		rege, '<$1> O texto dentro da tag "$1" é $2 $3 \n'));
